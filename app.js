async function getRequest() {
  ...
  // createProfileImage
  createProfileImage(req.data.viewer.avatarUrl);
  // createUserImage
  createUserImage(req.data.viewer.avatarUrl);
  // createUserInfo
  createUserInfo(req.data.viewer.name, req.data.viewer.twitterUsername);
  // bioInfo
  bioInfo(req.data.viewer.bio);
  // getRepoCount
  getRepoCount(req.data.viewer.repositories.totalCount);
  // Get Data from GITHUB
  const _data = req.data.viewer.repositories;

  // Loop through Data from GITHUB to Dynamically display content on the browser
  for (const data in _data.nodes) {
    const li = document.createElement('li');
    const obj = _data.nodes[data];
    const date = new Date(obj.updatedAt);
    const getShortMonth = date.toLocaleString('default', { month: 'short' });
...
<p class="repo-text"
  ...
  <span class="_m_r">
    ...
    >
      ...
      ></path></svg> ${obj.pullRequests.totalCount}
  </span>
